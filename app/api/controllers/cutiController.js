...
exports.ajukanCuti = async (req, res) => {
  	try{
        ...
        return error;
        const statuserr = httpStatus.INTERNAL_SERVER_ERROR;
        res.json(statuserr);
    }
}
...
exports.accCuti = async (req, res) => {
    ...
    Cuti.findOne({_id:id}, function(err, cuti){
      if(err){
        return err;
      }else {
...
exports.deleteCUti = async(req, res)=>{
  const id = req.params.id;
  Cuti.remove({_id:id}, function(err, cuti) {
    if(err){
      res.json({message:'can not find id of cuti!'});
      console.log(err);
    }else {
